<template>
  <FSelect :multiple="props.multiple" :loading="composedLoading" :placeholder="props.placeholder"
    v-model:value="computedValue" :options="pricingRuleOptions" :wide="props.wide" :clearable="props.clearable">
    <template #option-label="{ label }">
      <div class="pricing-rule-select__option-label">
        {{ label }}
      </div>
    </template>
  </FSelect>
